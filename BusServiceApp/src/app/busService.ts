...
export class BusService {
    ...
    searchByBusStopCode({busStopCode, serviceNo}){
        ...
        const url = `wss://mybustracker.herokuapp.com/arrival/${busStopCode}?${params.toString()}`
        //console.log('url', url)
    ...
    getBusServicesList(): Promise<any>{
        return this.http.get<BusService[]>(this.BASE_URL+'/busServices')
            .toPromise()
    }

    getBusServices(serviceNo: string): Promise<any>{
        return this.http.get<any>(this.BASE_URL+`/busServices/${serviceNo}`)
            .toPromise()
    }
